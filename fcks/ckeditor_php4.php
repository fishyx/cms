...
class CKEditor
{
	...
	function editor($name, $value = "", $config = array(), $events = array())
	{
        ...
        if (!empty($_config))
            $js .= "var editor_{$name} = CKEDITOR.replace('".$name."', ".$this->jsEncode($_config).");";
        else
            $js .= "var editor_{$name} = CKEDITOR.replace('".$name."');";

        $_SERVER['DOCUMENT_ROOT'] = str_replace('\\', '/', $_SERVER['DOCUMENT_ROOT']);
        $baseUrl = substr(__FILE__, 0, strpos(__FILE__, 'fcks'));
        $baseUrl = str_replace('\\', '/', $baseUrl);
        $baseUrl = str_replace($_SERVER['DOCUMENT_ROOT'], '', $baseUrl);
        $baseUrl = ltrim($baseUrl, '/');
        $js .= "CKFinder.setupCKEditor( editor_{$name}, '{$scriptName}/fcks/ckfinder/' )";
        $out .= $this->script($js);
