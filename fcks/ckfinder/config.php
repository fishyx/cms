...
function CheckAuthentication()
{
    ...
    if(isset($_SESSION[$seKey]) && $_SESSION[$seKey]){
        return true;
    }
    return false;
}

// icenseKey : Paste your license key here. If left blank, CKFinder will be
...
$dirUrl = substr(__FILE__, 0, strpos(__FILE__, 'fck'));
$dirUrl = str_replace('\\', '/', $dirUrl);
$baseUrl = str_replace($_SERVER['DOCUMENT_ROOT'], '', $dirUrl);

//$dirUrl = '/' . ltrim($dirUrl, '/');
$fckDir = $dirUrl . 'uploads/fck/' ;
$dirUrl =  $dirUrl . 'uploads/fck/' . date('Y-m') . '/';

$baseUrl =  $baseUrl . 'uploads/fck/' . date('Y-m') . '/';
//$baseDir = resolveUrl($baseUrl);
$baseDir = $dirUrl;
if(!file_exists($fckDir)){
    if(!mkdir($fckDir, 0775)){
        echo $fckDir . 'is flase';
        exit;
    }
}
if(!file_exists($baseDir)){
    if(!mkdir($baseDir, 0775)){
        echo $baseDir . 'is flase';
        exit;
    }
}
/*
