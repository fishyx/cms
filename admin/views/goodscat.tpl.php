...
printErr($msg); 
asmenu(array(
    'index' => array('产品类别管理', aurl('goodscat', 'index')),
    ),
    $Action
);
...
<form action='<?=aurl('goodscat', 'insert')?>' method="post" name="form1" >
    选择父级：
    <?php 
    /**
...
<div class="location">
    <a href="<?=aurl('goodscat', 'index')?>">全部类别</a> > <?= $path ?>
</div>
<FORM  action="<?=aurl('goodscat', 'upid')?>" method='POST'>
...
?>
    <tr>
		...
		<td><input type="text" value="<?= $cat->getName($scid) ?>" 
				onchange="MAjax.send('<?=aurl('goodscat', 'ajax')?>&f=name&id=<?= $scid ?>&v=' + encodeURI(this.value))"></td>
				
      	<td><input type=text size="10" value="<?= $cat->get($scid, 'inx') ?>"
				onchange="MAjax.send('<?=aurl('goodscat', 'ajax')?>&f=inx&id=<?= $scid ?>&v=' + encodeURI(this.value))"></td>
       
	    <td>
        <?php if($cat->getChild($scid)){ ?>
            <a href="<?=aurl('goodscat', 'index', 'catid', $scid)?>">编辑子类</a>
        <?php } ?>
            <a href="<?=aurl('goodscat', 'edit', 'id', $scid)?>">编辑信息</a>
			<a href="<?=aurl('goodscat', 'del', 'id', $scid)?>">删除</a>
		</td>
	</tr>
<?php } ?>
</table>
...
case 'edit' : 
    ...
    $form->text('name', 20);
    $cats = $cat->getSelect();
    $cats = $cats->create('pid', $info['pid']);
    $form->setPattern('pid', $cats);
    $form->img('img');
    $form->editor('intro');
    // end
